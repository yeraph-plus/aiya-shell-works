...
每个执行单元先在干净临时工作区运行，成功后覆盖合并到最终产物目录；最终目录中的旧文件不会进入本轮 `working_path`。输入目录与输出目录互相嵌套时会在复制前拒绝执行。
...
模块只应遍历 `ctx.working_path` 获取本轮输入；`ctx.output_dir` 是当前单元的干净临时产物目录，执行结束后路径会映射到最终输出。致命错误必须抛异常，`runtime.log(..., "error", ...)` 仅记录诊断信息，不会让工作流失败。
